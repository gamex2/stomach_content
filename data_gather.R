...
Stomach_fish_candat_melt <- setDT(melt(Stomach_fish_candat[, .(ct_catchid, SL, Year, Species, size_class, candat, ouklej, 
...
Stomach_fish_candat_melt <- Stomach_fish_candat_melt %>% 
  mutate(cannibal = case_when((prey == "candat" & prey_n >= 1) ~ 1, #condition 1
                              TRUE ~ 0)) #all other cases
Stomach_fish_candat_melt <- merge(Stomach_fish_candat_melt, fry_abundance[prey_sp == "candat"], by = "Year", all.x = T)
set.seed(3333)
summary(glm(data = Stomach_fish_candat_melt, formula = cannibal ~ size_class+Year+prey_n+Fry_abundance, family = binomial(link = "logit")))
...
sc.yoy <- ggplot(Stomach_fish_candat_melt2[size_class == "YOY"], aes(x = as.factor(Year), y = prey_n, fill = forcats::fct_rev(cannibal))) +
  geom_col(position = "stack") +
  geom_line(data=Stomach_fish_candat_melt2[size_class == "YOY"], group=1, mapping=aes(x=as.factor(Year),y=Fry_abundance/15), linewidth = 3)+
  scale_y_continuous(name = "Prey n", 
                     sec.axis = sec_axis(~.*15, name = "Fry n per hectare"))+
  labs(x="Year", fill = "cannibalism", linewidth = NULL, title="YOY")+
  theme(plot.title = element_text(size = 32, face = "bold"),
        axis.text.x = element_text(size = 28,angle = 45, hjust = 1.05, vjust = 1.05),
        axis.text.y = element_text(size = 28),
...
sc.old <- ggplot(Stomach_fish_candat_melt2[size_class == "older<300"], aes(x = as.factor(Year), y = prey_n, fill = forcats::fct_rev(cannibal))) +
  geom_col(position = "stack") +
  geom_line(data=Stomach_fish_candat_melt2[size_class == "older<300"], group=1, mapping=aes(x=as.factor(Year),y=Fry_abundance/23), linewidth = 3)+
  scale_y_continuous(name = "Prey n", 
                     sec.axis = sec_axis(~.*23, name = "Fry n per hectare"))+
  labs(x="Year", fill = "cannibalism", linewidth = NULL, title="older<300")+
  theme(plot.title = element_text(size = 32, face = "bold"),
        axis.text.x = element_text(size = 28,angle = 45, hjust = 1.05, vjust = 1.05),
        axis.text.y = element_text(size = 28),
...
sc.older <- ggplot(Stomach_fish_candat_melt2[size_class == "older>300"], aes(x = as.factor(Year), y = prey_n, fill = forcats::fct_rev(cannibal))) +
  geom_col(position = "stack") +
  geom_line(data=Stomach_fish_candat_melt2[size_class == "older>300"], group=1, mapping=aes(x=as.factor(Year),y=Fry_abundance/9), linewidth = 3)+
  scale_y_continuous(name = "Prey n", 
                     sec.axis = sec_axis(~.*9, name = "Fry n per hectare"))+
  labs(x="Year", fill = "cannibalism", linewidth = NULL, title="older>300")+
  theme(plot.title = element_text(size = 32, face = "bold"),
        axis.text.x = element_text(size = 28,angle = 45, hjust = 1.05, vjust = 1.05),
        axis.text.y = element_text(size = 28),
...
ggpubr::ggarrange(sc.yoy, sc.old, sc.older,
                  ncol = 2, nrow = 2,
                  common.legend = TRUE, legend = "bottom")
...
Stomach_fish_candat_size <- Stomach_fish_candat_size %>% 
  mutate(cannibal = case_when(prey_sp == "candat" ~ 1, #condition 1
                              TRUE ~ 0)) #all other cases
set.seed(3333)
...
ggplot(Stomach_fish_okoun_melt2, aes(x = as.factor(Year), y = prey_n, fill = forcats::fct_rev(cannibal))) +
  geom_col(position = "stack") +
  facet_wrap(~size_class, scales = "free") +
  labs(x="Year", y="Prey n", fill="Cannibalism")+
  theme(plot.title = element_text(size = 32, face = "bold"),
